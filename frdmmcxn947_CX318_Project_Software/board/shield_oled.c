...
/*
 * Sends the command CD followed by the buffer contents, of size
 */
void
OLED_Send(uint8_t* buffer, uint16_t size, uint8_t CD)
{
	LPI2C_MasterStart(SHIELD_OLED_I2C, OLED_ADDRESS, kLPI2C_Write);
	LPI2C_MasterSend(SHIELD_OLED_I2C, &CD, 1);
	LPI2C_MasterSend(SHIELD_OLED_I2C, buffer, size);
	LPI2C_MasterStop(SHIELD_OLED_I2C);
}
...
void OLED_SetPage(uint8_t page)
{
	...
	OLED_Send(&pageCommand, 1, OLED_COMMAND);
} /* OLED_SetPage() */
...
void OLED_SetSeg(uint8_t seg)
{
	...
	OLED_Send(segCommand, 2, OLED_COMMAND);
} /* OLED_SetSeg() */
...
void OLED_FillDisplay(uint8_t data)
{
	...
	OLED_SetPage(0);
	OLED_SetSeg(0);
	OLED_Send(frame, 1024, OLED_DATA);
} /* OLED_FillDisplay() */
...
void OLED_Print(const char* format, ...)
{
	...
	{
		OLED_LineWrap(); // check if line has reached end of screen
		switch(buffer[strChar])
		{
			...
			case '\x1b':
				...
				break;
			default:
				OLED_WriteChar(buffer[strChar], invert); // write character to display
...
{
	//TODO: fix inverted
	OLED_Send((uint8_t*)(&font[character][0]), 6, OLED_DATA); // write char with ASCII code 'character' from the font array
} /* writeChar() */
...
void OLED_LineWrap(void)
{
	if(currCharLoc[0] == 21)
	{
		...
		OLED_SetPage(currCharLoc[1] % 8); // only 8 pages, so set page to remainder of line division by 8
		/*
		...
		OLED_FillPage(0x00);
		OLED_SetPage(currCharLoc[1] % 8); //repeat in case segment has incremented to next page
		OLED_SetSeg(0);
	...
	if(currCharLoc[1] > 7)
	{
		OLED_Scroll(((currCharLoc[1] + 1) % 8) * 8); // set the scroll line at top of page to current page +1
	}
} /* lineWrap() */
...
void OLED_Scroll(uint8_t rows)
{
	...
	OLED_Send(scrollCommand, 2, OLED_COMMAND);
} /* scrollOLED() */
...
/*
 * This font is encoded to cp1252/Windows-1252. It is an extension or superset of IEC 8859-1 and
 * Unicode UTF-8 up to 0xFF, differing in that there are extra characters in the 0x80 to 0x9F range
 * in cp1252 instead of control sequences.
 */
char font[256][6]={
