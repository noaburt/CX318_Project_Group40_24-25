...
const uint32_t auw_hamm[31] = {41, 276, 512, 276, 41}; //Hamm =  long16(512* hamming(5)');
...
*               Thus, accurate SPO2 is recalculated and save uch_spo2_table[] per each ratio.
*
...
{
    int32_t k, i, s, m;
    int32_t un_only_once, n_npks, n_c_min;
    ...
    int32_t n_spo2_calc, n_y_dc_max, n_x_dc_max;
    ...
    for(k = 0; k < BUFFER_SIZE - MA4_SIZE - 2; k++) { an_dx[k] = (an_dx[k] + an_dx[k+1]) / 2; }
    ...
    for (i = 0 ; i < BUFFER_SIZE - HAMMING_SIZE - MA4_SIZE - 2; i++) {
        ...
        for(k = i; k < i + HAMMING_SIZE; k++) { s -= an_dx[k] * auw_hamm[k-i]; }
    ...
    *pn_npks = 0;
    ...
    while (i < n_size - 1) {
        if (pn_x[i] > n_min_height && pn_x[i] > pn_x[i-1]) {            // find left edge of potential peaks
            ...
            while (i + n_width < n_size && pn_x[i] == pn_x[i+n_width]) { n_width++; }   // find flat peaks
